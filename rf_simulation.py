...
S_mean = np.mean(s_n_mean_z(s_n_contiguous(gz=gz, fa=fa_corrected)))
print("Mean signal using measured and corrected flip angle = " +
      str(np.round(S_mean, 3)))
S_0_mean = np.mean(s_n_mean_z(s_n_contiguous_no_decay(gz=gz, fa=fa_corrected)))
print("Mean S0 using measured and corrected flip angle = " +
      str(np.round(S_0_mean, 3)))

fa = np.deg2rad(fa_corrected)
numerator = integral_contiguous_no_decay(fa, gz)
denominator = integral_contiguous(fa, gz) / n_excitations
...
m_0_recovered = S_0_recovered/integral_contiguous_no_decay(fa, gz)
print("Recovered M0 = " +
      str(np.round(m_0_recovered, 3)))
